...
object AMRtoDTmapping {
  val dt = DependencyTree("NATO allies have signed an agreement to fund a research center to bolster defenses against cyber attacks..")
                                                  ...
                                                  //| im.crf.ser.gz ... done [6.4 sec].
                                                  //| Loading classifier from edu/stanford/nlp/models/ner/english.muc.7class.dists
                                                  //| im.crf.ser.gz ... done [2.9 sec].
                                                  ...
                                                  //| SwappedArcs:	Set()
                                                  //| DeletedNodes:	List()
  dt.nodeLemmas                                   //> res0: Map[Int,String] = Map(5 -> a, 10 -> research, 14 -> defens, 1 -> nato,
                                                  //|  6 -> agreement, 9 -> a, 13 -> bolster, 2 -> ally, 17 -> attack, 12 -> to, 7
                                                  //|  -> to, 3 -> have, 16 -> cyber, 11 -> center, 8 -> fund, 4 -> sign, 15 -> ag
                                                  //| ainst)
  dt.nodePOS                                      //> res1: Map[Int,String] = Map(5 -> DT, 10 -> NN, 14 -> NNS, 1 -> NNP, 6 -> NN,
                                                  //|  9 -> DT, 13 -> VB, 2 -> NNS, 17 -> NNS, 12 -> TO, 7 -> TO, 3 -> VBP, 16 -> 
                                                  //| NN, 11 -> NN, 8 -> VB, 4 -> VBN, 15 -> IN)
  dt.nodeNER                                      //> res2: Map[Int,String] = Map(5 -> O, 10 -> O, 14 -> O, 1 -> ORGANIZATION, 6 -
                                                  //| > O, 9 -> O, 13 -> O, 2 -> O, 17 -> O, 12 -> O, 7 -> O, 3 -> O, 16 -> O, 11 
                                                  //| -> O, 8 -> O, 4 -> O, 15 -> O)
  dt.nodes foreach (_ match { case (node, word) => println(s"$node -> $word has parents ${dt.edgesToParents(node)}") })
                                                  ...
                                                  //| 5 -> an has parents ArrayBuffer((6,5))
                                                  //| 10 -> research has parents ArrayBuffer((11,10))
                                                  //| 14 -> defenses has parents ArrayBuffer((13,14))
                                                  //| 1 -> NATO has parents ArrayBuffer((2,1))
                                                  //| 6 -> agreement has parents ArrayBuffer((4,6))
                                                  //| 9 -> a has parents ArrayBuffer((11,9))
                                                  //| 13 -> bolster has parents ArrayBuffer((8,13))
                                                  //| 2 -> allies has parents ArrayBuffer((4,2))
                                                  //| 17 -> attacks has parents ArrayBuffer((15,17))
                                                  //| 12 -> to has parents ArrayBuffer((13,12))
                                                  //| 7 -> to has parents ArrayBuffer((8,7))
                                                  //| 3 -> have has parents ArrayBuffer((4,3))
                                                  //| 16 -> cyber has parents ArrayBuffer((17,16))
                                                  //| 11 -> center has parents ArrayBuffer((8,11))
                                                  //| 8 -> fund has parents ArrayBuffer((6,8))
                                                  //| 4 -> signed has parents ArrayBuffer((0,4))
                                                  //| 15 -> against has parents ArrayBuffer((13,15))
  dt.nodes foreach (_ match { case (node, word) => println(s"$node -> $word has depth ${dt.depth(node)}") })
                                                  //> 0 -> ROOT has depth 0
                                                  //| 5 -> an has depth 3
                                                  //| 10 -> research has depth 5
                                                  //| 14 -> defenses has depth 5
                                                  //| 1 -> NATO has depth 3
                                                  //| 6 -> agreement has depth 2
                                                  //| 9 -> a has depth 5
                                                  //| 13 -> bolster has depth 4
                                                  //| 2 -> allies has depth 2
                                                  //| 17 -> attacks has depth 6
                                                  //| 12 -> to has depth 5
                                                  //| 7 -> to has depth 4
                                                  //| 3 -> have has depth 2
                                                  //| 16 -> cyber has depth 7
                                                  //| 11 -> center has depth 4
                                                  //| 8 -> fund has depth 3
                                                  //| 4 -> signed has depth 1
                                                  //| 15 -> against has depth 5
  dt.getNodesBetween(4, 9)                        //> res3: Seq[Int] = List(9, 11, 8, 6, 4)
  dt.getPathBetween(4, 9)                         //> res4: String = VBN-dobj-NN-vmod-VB-dobj-NN-det-DT
  dt.getNodesBetween(9, 4)                        //> res5: Seq[Int] = List(4, 6, 8, 11, 9)
  dt.getPathBetween(9, 4)                         //> res6: String = DT-det-NN-dobj-VB-vmod-NN-dobj-VBN
  dt.getNodesBetween(0, 3)                        //> res7: Seq[Int] = List(3, 4, 0)
  dt.getPathBetween(0, 3)                         //> res8: String = XX-root-VBN-aux-VBP
  dt.getNodesBetween(2, 8)                        //> res9: Seq[Int] = List(8, 6, 4, 2)
  dt.getPathBetween(2, 8)                         //> res10: String = NNS-nsubj-VBN-dobj-NN-vmod-VB
  dt.getPathBetween(4, 4)                         //> res11: String = VBN-ERR

  dt.subGraph(4)                                  //> res12: Set[Int] = Set(5, 10, 14, 1, 6, 9, 13, 2, 17, 12, 7, 3, 16, 11, 8, 4,
                                                  //|  15)
  dt.subGraph(9)                                  //> res13: Set[Int] = Set(9)
  dt.subGraph(0)                                  //> res14: Set[Int] = Set(0, 5, 10, 14, 1, 6, 9, 13, 2, 17, 12, 7, 3, 16, 11, 8,
                                                  //|  4, 15)
  dt.subGraph(3)                                  //> res15: Set[Int] = Set(3)

  dt.getNeighbourhood(3, 1)                       //> res16: Set[Int] = Set(4)
  dt.getNeighbourhood(3, 2)                       //> res17: Set[Int] = Set(0, 6, 2, 4)
  dt.getNeighbourhood(3, 3)                       //> res18: Set[Int] = Set(0, 5, 1, 6, 2, 8, 4)
  dt.getNeighbourhood(3, 4)                       //> res19: Set[Int] = Set(0, 5, 1, 6, 13, 2, 7, 11, 8, 4)
  dt.getNeighbourhood(5, 1)                       //> res20: Set[Int] = Set(6)
  dt.getNeighbourhood(5, 2)                       //> res21: Set[Int] = Set(4, 8, 6)
  dt.getNeighbourhood(5, 3)                       //> res22: Set[Int] = Set(0, 6, 13, 2, 7, 3, 11, 8, 4)
  dt.getNeighbourhood(5, 4)                       //> res23: Set[Int] = Set(0, 10, 14, 1, 6, 9, 13, 2, 12, 7, 3, 11, 8, 4, 15)
  
  dt.getDistanceBetween(4, 9)                     //> res24: Int = 4
  dt.getDistanceBetween(9, 4)                     //> res25: Int = 4
  dt.getDistanceBetween(0, 3)                     //> res26: Int = 2
