...
object ClassicTransitionSystem extends TransitionSystem[Sentence, ClassicAction, ClassicTransitionState] {
  ...
  def actions(): Array[ClassicAction] = NextNode.all ++ NextEdge.all ++ AddParent.all ++ Array(DeleteNode, CreateFragment, ReversePolarity, DoNothing)

  def actions(state: ClassicTransitionState) = {
    if (state.nodesToProcess.isEmpty)
      ...
      state.phase match {
        ...
        case 4 =>
          val nextEdgeIndices = if (state.nodePair == (0, 0)) Set(0) else {
            val concept1 = if (state.currentGraph.nodeLemmas(state.nodePair._1) == "##") "##" else state.currentGraph.nodes(state.nodePair._1)
            val concept2 = if (state.currentGraph.nodeLemmas(state.nodePair._2) == "##") "##" else state.currentGraph.nodes(state.nodePair._2)
            ...
            val permissibleEdges2 = (edgeKeys2 flatMap { concept => edgesPerConcept.getOrElse(concept, Set()) }) map { i => -i }
          ...
          val output: Array[ClassicAction] = (nextEdgeIndices map { i => NextEdge(i) }).toArray
          output
        case 5 => 
          Array[ClassicAction](ReversePolarity, DoNothing)
        case _ =>
          Array.empty[ClassicAction]
      }
    }
  ...
  override def isTerminal(state: ClassicTransitionState): Boolean = state.phase == 6
